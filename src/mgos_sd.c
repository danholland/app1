...
void mgos_sd_read_file(char *filename)
{
  FILE *f = fopen(filename, "r");
  if (f == NULL)
  {
    LOG(LL_ERROR, ("Failed to open '%s' for reading", filename));
    return;
  }
  char l[256];
  while (fgets(l, sizeof(l), f))
  {
    LOG(LL_INFO, ("%s", strtok(l, "\n")));
  }
  fclose(f);
  return;
}

void mgos_sd_write_line(char *filename, char *line)
{
  FILE *f = fopen(filename, "a");
  if (f == NULL)
  {
    LOG(LL_ERROR, ("Failed to open '%s' for appending", filename));
    return;
  }
  fprintf(f, "%s!\n", line);
  fclose(f);
...
void mgos_sd_write_file(char *filename, char *content)
{
  FILE *f = fopen(filename, "w");
  if (f == NULL)
  {
    LOG(LL_ERROR, ("Failed to open '%s' for writing", filename));
    return;
  }
  fprintf(f, "%s!", content);
  fclose(f);
...
void mgos_sd_rename_file(char *source, char *dest)
{
  unlink(dest);
...
void mgos_sd_delete_file(char *filename)
{
  if (unlink(filename) != 0)
  {
    LOG(LL_ERROR, ("Delete failed"));
    return;
  }
  return;
}
